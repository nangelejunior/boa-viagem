...
import br.com.casadocodigo.boaviagem.dao.DatabaseHelper;
import static br.com.casadocodigo.boaviagem.provider.BoaViagemContract.*;
...
public class BoaViagemProvider extends ContentProvider {
	...
	static {
		...
		uriMatcher.addURI(AUTHORITY, GASTO_PATH + "/" + VIAGEM_PATH + "/#",
				GASTOS_VIAGEM_ID);
	}
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		case VIAGEM_ID:
			selection = Viagem._ID + " = ?";
			selectionArgs = new String[] { uri.getLastPathSegment() };
			return database.update(VIAGEM_PATH, values, selection,
					selectionArgs);
		...
		case GASTO_ID:
			...
			selectionArgs = new String[] { uri.getLastPathSegment() };
			return database
					.update(GASTO_PATH, values, selection, selectionArgs);
